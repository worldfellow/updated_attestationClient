      ...
      <div class="card-body">
        <form [formGroup]="hrdForm" (ngSubmit)="saveHrd()">
          ...
          <div class="row">
            ...
            <div class="col-md-8">
              <input type="text" [(ngModel)]="cgpa_no" class="form-control" (keyup)="checkCgpa($event, 'cgpi')" formControlName="cgpaNo" [ngClass]="{'is-invalid': cgpaNo.invalid && (cgpaNo.dirty || cgpaNo.touched)}">
              <p>OR</p>
            </div>
          </div>
          ...
          <div class="row">
            ...
            <div class="col-md-8">
              <input type="text" [(ngModel)]="cgpi_no" class="form-control" (keyup)="checkCgpa($event, 'cgpi')" formControlName="cgpiNo" [ngClass]="{'is-invalid': cgpiNo.invalid && (cgpiNo.dirty || cgpiNo.touched)}">
            </div>
          </div>
...
export class AddHrdDialogComponent {
  ...
  saveHrd() {
    ...
    if (this.function_type) {
      ...
      this.api.updateAllHrd(this.formData, this.user_id, this.function_type, this.degree_type, this.secondlastSem, this.lastSem, this.purpose_name, this.hrd_id).subscribe((data: any) => {
        if (data['status'] == 200) {
          this.dismiss();
